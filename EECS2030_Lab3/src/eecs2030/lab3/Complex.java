...
public final class Complex {
	...
	public Complex add(Complex c) {
		double newRe = this.real + c.real;
		double newIm = this.imag + c.imag;
	...
	public Complex multiply(Complex c) {
		...
		double newRe = (this.real * c.real) - (this.imag * c.imag);
		double newIm = (this.imag * c.real) + (this.real * c.imag);
...
//		return Math.sqrt((Math.pow(this.real, 2) + Math.pow(this.imag, 2)));   won't work due to overflow issues.
	...
	public String toString() {
		...
		if(this.imag < 0) {
			double newImag = -1.0 * this.imag;
			return this.real + " - " + newImag+"i"; 
		}
		else {
	...
	public static Complex valueOf(String s) {
		...
		if(parts.get(2).charAt(parts.get(2).length() - 1) != 'i') {
			...
			String imagRe = parts.get(2).substring(0, parts.get(2).length() - 1);
			imag = Double.valueOf(imagRe);
		}
